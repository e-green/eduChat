...
(function (exports) {
    ...
    function encodeEntities(value) {
        return value.replace(/&/g, '&amp;').replace(surrogatePairRegexp, function (value) {
    ...
    function maths(text) {
        var mathsPattern = /maths:\/\/*/;
        // console.log();
        if (mathsPattern.test(text)) {
            var latext = text.replace(mathsPattern, "");

            // var htmlLatext = '<span class="mathLatext">' + latext + '</span>';
            var mathFieldSpan = $('<span>' + latext + '</span>');
            var mathField = MQ.StaticMath(mathFieldSpan[0]);

            mathField.reflow();

            return mathFieldSpan;
